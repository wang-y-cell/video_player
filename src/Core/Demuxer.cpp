...
bool Demuxer::open(const std::string& url) {
    ...
    if (info_ret < 0) {
        ...
        LOG_ERROR("Demuxer", "读取流信息失败: " << last_error_);
        return false;
    }
    ...
    LOG_INFO("Demuxer", "流信息就绪，音频流索引=" << audio_index_ << "，视频流索引=" << video_index_);
...
bool Demuxer::seekSeconds(double target_seconds) {
    ...
    if (!fmt_ctx_) {
        last_error_ = "格式上下文为空";
        LOG_ERROR("Demuxer", "跳转失败: " << last_error_);
        return false;
    }
    ...
    LOG_INFO("Demuxer", "开始跳转到 " << target_seconds << " 秒");
    ...
    if (ret < 0) {
        last_error_ = ff::errStr(ret);
        LOG_ERROR("Demuxer", "执行跳转失败: " << last_error_);
        return false;
    }

    avformat_flush(fmt_ctx_);
    seek_generation_.fetch_add(1);
    LOG_INFO("Demuxer", "跳转成功");
    return true;
}
